...
struct PerformanceStats {
    ...
    // memory tracking
    size_t peak_memory_used_bytes = 0;
    size_t initial_free_mem = 0;
    size_t initial_total_mem = 0;
};

// 空的KV cache统计占位 - src_origin中没有prefix_cache实现
struct DummyPrefixCache {
    size_t getHitCount() const { return 0; }
    size_t getTotalLookupCount() const { return 0; }
    double getHitRate() const { return 0.0; }
};

static void updatePeakMemory(PerformanceStats& stats) {
    size_t free_mem, total_mem;
    cudaMemGetInfo(&free_mem, &total_mem);
    size_t used_mem = total_mem - free_mem;
    if (used_mem > stats.peak_memory_used_bytes) {
        stats.peak_memory_used_bytes = used_mem;
    }
}

static void printStats(const PerformanceStats& stats, const DummyPrefixCache* prefix_cache) {
    double total_time_ms = std::chrono::duration<double, std::milli>(stats.end_time - stats.start_time).count();
    ...
    std::cout << "[1] PREFILL TIME\n";
    std::cout << "  Total prefill calls:  " << stats.prefill_calls << "\n";
    std::cout << "  Total prompt tokens:  " << stats.total_prompt_tokens << "\n";
    std::cout << "  Total prefill time:   " << stats.total_prefill_time_ms << " ms\n";
    if (stats.total_prompt_tokens > 0) {
        std::cout << "  Avg per prompt token: " << (stats.total_prefill_time_ms / stats.total_prompt_tokens) << " ms/token\n";
    }
    std::cout << "\n";

    std::cout << "[2] KV-CACHE HIT RATE\n";
    std::cout << "  N/A (prefix cache not implemented in src_origin)\n";
    std::cout << "\n";

    std::cout << "[3] MEMORY PEAK\n";
    std::cout << "  Peak GPU memory used: " << (stats.peak_memory_used_bytes / (1024 * 1024)) << " MB  (" << (stats.peak_memory_used_bytes / (1024ULL * 1024 * 1024)) << " GB)\n";
    if (stats.initial_total_mem > 0) {
        std::cout << "  Total GPU memory:     " << (stats.initial_total_mem / (1024 * 1024)) << " MB  (" << (stats.initial_total_mem / (1024ULL * 1024 * 1024)) << " GB)\n";
        std::cout << "  Memory utilization:   " << (100.0 * stats.peak_memory_used_bytes / stats.initial_total_mem) << "%\n";
    }
    std::cout << "\n";

    std::cout << "[4] END-TO-END THROUGHPUT\n";
    std::cout << "  Total generated tokens: " << stats.total_generated_tokens << "\n";
    std::cout << "  Total runtime:          " << total_time_ms << " ms  (" << total_time_s << " s)\n";
    if (total_time_s > 0) {
        std::cout << "  Throughput:             " << (stats.total_generated_tokens / total_time_s) << " tokens/s\n";
    }
    std::cout << "\n";

    std::cout << "[5] TPOT (Time Per Output Token)\n";
    std::cout << "  Total decode steps:     " << stats.decode_steps << "\n";
    std::cout << "  Total decode time:      " << stats.total_decode_time_ms << " ms\n";
    if (stats.total_generated_tokens > 0) {
        std::cout << "  TPOT (end-to-end):      " << (total_time_ms / stats.total_generated_tokens) << " ms/token\n";
    }
    if (stats.total_generated_tokens > 0 && stats.total_decode_time_ms > 0) {
        std::cout << "  TPOT (decode-only):     " << (stats.total_decode_time_ms / stats.total_generated_tokens) << " ms/token\n";
    }
    std::cout << "\n";
    std::cout << "========================================\n";
}
...
constexpr int DEFAULT_MAX_NEW_TOKENS_GENERATED = 0; // 0 means unlimited for apples-to-apples benchmark runs
...
constexpr int MAX_SEQ_LEN = 8192;            // TODO: make it tunable
constexpr int BATCH_SIZE = 2;                // TODO: not even close to being good, it's just here to have batching
constexpr int MAX_PROMPT_LEN = 2048;         // keep a safe long-input working set for prefill buffers
constexpr int MAX_BUFFER_SIZE = std::max(MAX_PROMPT_LEN, BATCH_SIZE);
constexpr int BLOCK_SIZE = 128; // TODO: tunable as well, defined the size of a single page in pagedattn
...
inline bool validatePromptTokens(const std::vector<int> &tokens, int prompt_len)
{
    if (prompt_len > MAX_PROMPT_LEN)
    {
        std::cerr << "Received too many tokens in one prefill chunk: " << prompt_len
                  << " > " << MAX_PROMPT_LEN << "\n";
        return false;
    }
    for (int token : tokens)
    {
        ...
        {
            std::cerr << "Token id out of range for embedding lookup: " << token << "\n";
            return false;
        }
    }
    return true;
...
int main(int argc, char *argv[])
{
    ...
    // 记录初始内存状态（模型加载后）
    cudaMemGetInfo(&stats.initial_free_mem, &stats.initial_total_mem);
    ...
    // --- 1. 定义我们的长前缀 (模拟 512 个 token) ---
    std::vector<int> long_common_prefix(512);
    for (int i = 0; i < 512; ++i)
    {
        long_common_prefix[i] = 100;
    }

    std::queue<std::vector<int>> queue;
    for (int i = 0; i < BATCH_SIZE; ++i)
    {
        std::vector<int> full_prompt = long_common_prefix;
        full_prompt.push_back(1000 + i);
        queue.push(full_prompt);
    }
    ...
    for (int slot = 0; slot < is_slot_free.size() && !queue.empty(); ++slot)
    {
        ...
        // 记录prefill前的queue大小和prompt，用于统计token数
        size_t queue_size_before = queue.size();
        const std::vector<int>& next_prompt = queue.front();
        int tokens_in_this_call = static_cast<int>(next_prompt.size());
        ...
        stats.total_prefill_time_ms += std::chrono::duration<double, std::milli>(prefill_end - prefill_start).count();
        stats.prefill_calls++;
        stats.total_prompt_tokens += tokens_in_this_call;
        updatePeakMemory(stats);
    }
    ...
    while (true) // exit condition irrelevant for now, since it's an inference server that's supposed to run foreveeer!!!
    {
        ...
        int tokens_generated_this_step = num_active_slots;
        ...
        stats.total_decode_time_ms += std::chrono::duration<double, std::milli>(decode_end - decode_start).count();
        stats.decode_steps++;
        stats.total_generated_tokens += tokens_generated_this_step;
        updatePeakMemory(stats);
    }

    stats.end_time = std::chrono::high_resolution_clock::now();
    updatePeakMemory(stats);
    printStats(stats, &dummy_cache);
